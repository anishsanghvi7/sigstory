...
if (!is.null(pairwise_sim_param) && file.exists(pairwise_sim_param)) {
  ...
  if (is.null(pairwise_sim_param))  {
    similarity_link <- markdown(glue("No similar samples found as no similarity file was inputted. Please refer to ",
                                     documentation_link))
  } else if (!file.exists(pairwise_sim_param)) {
    similarity_link <- markdown(glue("No similar samples found")
  }
...
if (total_mutations > 0 && length(good_sigs) != 0) {
    if (nrow(parquet_db) != 0) {
      ...
      card(
        ...
        markdown(glue("NOTE: Parquet Path not inputted. To pass through a parquet database refer to the ", documentation_link))
      )
    }
