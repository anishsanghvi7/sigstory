...
if (file.exists(dimensionality_reduction)) {
...
create_tsne <- function() {
  ...
  # contrib_tsne_df <- decomp |>
  #   filter(class == 'SBS96', count != 0) |>
  #   select(sampleId, channel, fraction)
  # 
  # wide_tsne <- contrib_tsne_df |>
  #   pivot_wider(names_from = channel,
  #               values_from = fraction,
  #               values_fill = 0)
  # 
  # wide_tsne_matrix <- as.matrix(wide_tsne[2:ncol(wide_tsne)])
  # 
  # set.seed(1)
  # tsne_out <- Rtsne(wide_tsne_matrix, perplexity = 30)
  # 
  # tsne_plot <- data.frame(x = tsne_out$Y[,1], 
  #                         y = tsne_out$Y[,2])
  # 
  # tsne_plot <- tsne_plot |>
  #   mutate(sampleId = wide_tsne$sampleId) |>
  #   mutate(is_sample_of_interest = ifelse(sample_of_interest == sampleId, 1, 0))
  #   
  # colour_disease <- tsne_plot |>
  #   left_join(sample_type_of_interest)
  # 
  # colour_disease$disease <- str_to_title(colour_disease$disease)
  # 
  # ######################
  # ## Generate colours ##
  # ######################
  # 
  # num_categories <- length(unique(colour_disease$disease))
  # color_palette <- generate_distinct_colors(num_categories)
  # 
  # ##########################
  # ## Create the TSNE Plot ##
  # ##########################
  # tsne_tooltip <- paste(colour_disease$sampleId, "-", colour_disease$disease)
  # 
  # sample_of_interest_matrix <- subset(colour_disease, is_sample_of_interest == 1)
  # other_samples <- subset(colour_disease, is_sample_of_interest == 0)
  # 
  # tsne_plotly <- plot_ly() %>%
  #   add_trace(data = other_samples,
  #             x = ~x, 
  #             y = ~y, 
  #             text = ~paste(sampleId, "-", disease),
  #             hoverinfo = "text",
  #             type = "scatter",
  #             mode = "markers",
  #             color = ~disease,
  ...
  #                          sample_of_interest_matrix$sampleId, sep="\n")
  #   ) %>%
...
create_residual_model <- function() {
  ...
  string_title <- glue("Note: If a few discrepancies exceed the threshold, being 7.5% of total mutations (±{residual_thresh}) the data may be unreliable. Also if the cosine similarity is not close to 1, then the analysis is most likely invalid.")
...
generate_observed_similar <- function(sample_id) {
  file <- gsub(sample_of_interest, sample_id, tally_param)
  
  if(!file.exists(file)) {
    observed_plot <- ggplot() +
      annotate("text", x = 0.5, 
               y = 0, 
               label = "No mutations found for this sample", 
               size = 2) +
      theme_void()
