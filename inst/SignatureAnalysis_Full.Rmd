...
exposures_param <- params$exposures
...
# Exposure
if (file.exists(exposures_param)) {
  exposure <- readr::read_csv(exposures_param)
} else {
...
if (!is.null(pairwise_sim_param) && file.exists(pairwise_sim_param)) {
  pairwise_sim <- readr::read_csv(pairwise_sim_param)
} else {
  if (is.null(pairwise_sim_param))  {
    similarity_link <- markdown(glue("No similar samples found as no similarity .csv file was inputted. Please refer to ",
                                     documentation_link))
  } else if (!file.exists(pairwise_sim_param)) {
    similarity_link <- glue("No similar samples found")
  }
...
if (!is.null(dimensionality_reduction) && file.exists(dimensionality_reduction)) {
  tsne_df <- readr::read_csv(dimensionality_reduction)
} else {
...
create_box_and_heatmap <- function() {
  ...
  if (dim(exposure_boot)[1] == 0) {
    bad_sample_box <- value_box(
        title = "Error",
        value = "No optimal model contributions for the sample",
        theme = "danger",
...
create_tsne <- function() {
  if (is.null(dimensionality_reduction) == 0) {
    return(tsne_link)
  }
...
reconstructed_against_observed <- function() {
  if (nrow(decomp) == 0 || nrow(exposure) == 0) {
...
if (total_mutations > 0 && length(good_sigs) != 0) {
    if (nrow(parquet_db) != 0) {
      ...
      card(
        ...
        markdown(glue("Cohort Contribution plots can not be displayed as parquet path not found/inputted. Please refer to ", documentation_link))
      )
    }
